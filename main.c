...
int main(void) {
    ...
    while(1){
        ...
        if(adc_addlog == 1)
        {
            int l, input_voltage, output_voltage;
            adc_addlog = 0;
            GPIO_setOutputLowOnPin( GPIO_PORT_P1, GPIO_PIN0);				//turn off led (1.0)

            setupADC1();
            for(l = 0;l < SAMPLES ; l++){
                GPIO_setOutputHighOnPin( GPIO_PORT_P4, GPIO_PIN5);			//turn on led (1.0)to indicate ADC conversion started
                ADCstartConv();
                ...
                ADCstopConv();
                GPIO_setOutputLowOnPin( GPIO_PORT_P4, GPIO_PIN5);			//turn off led (1.0) to indicate ADC conversion completed
            }
            input_voltage = takeSamples();
...
//			   GPIO_setOutputLowOnPin( GPIO_PORT_P4, GPIO_PIN5);			//turn off led (1.0) to indicate ADC conversion completed
//		   }
//		   output_voltage = takeSamples();

#if(1)
            //uart transmit the adc converted value
            sprintf(str,"%d,", input_voltage);
			myuart_tx_byte(',');
...
#if(0)
			//log using datlog.c
			data_buffer(input_voltage, output_voltage);
#endif
      }
    }
}
